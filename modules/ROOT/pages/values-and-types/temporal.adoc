...
Below are examples of parsing instant values using various temporal functions.
More information about these temporal functions can be found xref::functions/temporal/index.adoc[here].

.+datetime+
======

Parsing a `ZONED DATETIME` using the _calendar date_ format:
...
Parsing a `LOCAL DATETIME` using the _ordinal date_ format:
...
Parsing a `DATE` using the _week date_ format:
