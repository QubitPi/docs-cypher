...
A subpath variable (a path variable declared inside a parenthesized path pattern expression) can only be used if a xref:patterns/reference.adoc#shortest-paths[shortest paths] mode is also specified.
...
To avoid introducing xref::patterns/reference.adoc#variable-length-relationships-equijoins[equijoins] between the two instances of `x`, and between the two instances of `z`, the variables are replaced with a set of fresh variables inside each iteration:
...
See xref:patterns/reference.adoc#path-patterns[path patterns] for details on where in the path pattern to include the path selector.
